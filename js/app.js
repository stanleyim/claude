...
function showHoliday(msg){
  ...
  // 배지 변경
  if(el('liveBadge')){
    el('liveBadge').textContent = '● 휴장 (최근 데이터)';
    el('liveBadge').style.color = '#94a3b8';
  }
}
...
function renderCards(list){
  ...
  el('grid').innerHTML = list.map((s,i)=>`
    <div class="card">
      <div style="font-weight:700">${i+1}. ${s.name}</div>
      <div style="font-size:.75rem;color:#888">${s.code}</div>
      <div style="margin-top:6px;font-size:1.1rem">${s.score?.total||0}점</div>
    </div>
  `).join('');
}
